...
summary: "Análisis de la instalación de la temática de género en la cámara de Representantes del Parlamento uruguayo en la legislatura 2015-2020"
image:
  caption: ''
  focal_point: ''
  output:
...
entrenamiento <- corpus(train,text_field = "speech")%>%
  corpus_trimsentences(min_length = 13) # quito las intervenciones que tienen menos de 13 palabras 

entrenamiento_dfm <- dfm(entrenamiento,
                         tolower = TRUE, #transforma todo a minúscula
                         remove_punct = TRUE, #elimina puntuación  
                         remove_numbers = TRUE,  #elimina números 
                         remove = c(stopwords("spanish"),palabras_drop), #elimina stopwords pre-cargadas en quanteda y palabras específicas de interés
                         verbose = TRUE) %>% #imprime un resumen del proceso
  dfm_remove(min_nchar=3)%>% # elimino las palabras que tienen 1 y 2 caracteres
  dfm_trim(min_termfreq = 90) #le indico que se quede con téminos que aparezcan al menos 90 veces (Esto se va calibrando en la marcha!). Me permite sacar los términos que se usan muy poco. 
...
textplot_wordcloud(dfm(entrenamiento_dfm, groups = "genero"), min.count = 10,max_words = 300,random.order = TRUE,rot.per = .25, colors = c("#954342","#e76363"),comparison = T)
...
Por su parte, si lo relativizamos según el peso de cada partido en términos de cantidad de legisladorxs, y consideramos la cantidad de menciones por legisladorx, el __Partido Colorado__ es el que cuenta con mayor peso relativo.


```{r echo=FALSE, message=FALSE, warning=FALSE}

part_ratio=base_final %>%
  group_by(Partido) %>%
summarise(ratio= n()/length(unique(legislator)))%>%
  ggplot(aes(x = reorder(Partido, -ratio) , y = ratio,group = 1)) + 
  geom_bar(size=1, stat="identity",fill = c("#954342","#4a2524","#e76363","#bd5352","#6e3432"),width = 0.6) +
#  scale_y_continuous(limits = c(0, 100))+ 
  scale_fill_gradient(low = "white", high = "red")+
  theme(axis.text.x = element_text(size=10,family = "Impact"), 
        axis.text.y = element_text(size=10,family = "Impact"),
...
Por último, queremos ver que legisladorxs son los que tienen más intervenciones referidas al género vemos que la diputada __Gloria Rodriguez__ es la que acumula más menciones, y que la gran mayoría son mujeres `r emo::ji("female")`.Esto ratifica la idea de que las mujeres son las que más instalan la temática de género, reafirmando la necesidad de paridad y participación equilibrada en la representación parlamentaria. 
